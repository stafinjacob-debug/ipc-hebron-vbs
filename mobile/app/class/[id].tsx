...
export default function ClassDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await apiFetch<ClassPayload>(
      `/api/mobile/v1/seasons/${seasonId}/classes/${id}${campDateQuery}`,
      { token },
    );
    setData(res);
  ...
  const multiDay = data.attendance?.multiDayCheckInEnabled && (data.attendance?.campDates.length ?? 0) > 0;
  const dayLabel =
    data.attendance?.campDates.find((d) => d.key === (campDate ?? data.attendance?.campDate))?.label ??
    'Today';
    ...
    <FlatList
      ...
      ListHeaderComponent={
        <View style={styles.header}>
          ...
          <Text style={styles.meta}>
            {data.counts.checkedIn}/{data.counts.enrolled} checked in · {dayLabel} · cap{' '}
            {data.class.capacity}
          </Text>
          ...
          {multiDay ? (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.dayRow}>
              {(data.attendance?.campDates ?? []).map((day) => {
                const selected = (campDate ?? data.attendance?.campDate) === day.key;
                return (
                  <Pressable
                    key={day.key}
          ...
          {readOnly ? (
            <Text style={styles.muted}>Tap a student to view details (read-only).</Text>
          ) : null}
        </View>
      }
      renderItem={({ item }) => (
